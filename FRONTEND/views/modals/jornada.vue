<template>
	...
	>
		<q-card style="width: 300px">
			<q-card-section>
				<div class="text-h6">Jornada</div>
			</q-card-section>

			<q-card-section style="max-height: 50vh" class="scroll">
				<q-form class="q-gutter-md">
					<q-select
						v-model="form._id_ctlg_jornadas"
...
<script>
	export default {
		...
		methods: {
			...
			guardar() {
				if (
					this.form._id_ctlg_jornadas !== null &&
					this.form._id_ctlg_jornadas !== ""
				) {
					this.$q.loading.show();
					_.rpost("jornadas", null, this.form)
						.then((r) => {
							this.close();
							this.$parent.listaJornadas();
							this.$q.notify({
								type: "positive",

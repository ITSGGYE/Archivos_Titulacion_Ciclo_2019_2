<template>
	<q-page class="q-pa-md">
		...
		<q-table
			...
			title="Insumos"
			:data="insumos.data"
			:columns="insumos.columns"
			row-key="id"
			:filter="filter"
			:loading="insumos.loading"
		>
			...
			<template v-slot:body="props">
				...
				>
					<q-td colspan="100%">
						<q-select
							v-model="id_ctlg_insumoss"
							:options="options.insu"
							label="Insumos"
							emit-value
							map-options
						>
	...
	export default {
		...
		methods: {
			excel() {
				this.report(
					"Reporte Insumos ",
					this.insumos.columns,
					this.insumos.data
				);
			},
			insu() {
				return _.rget("insumos/ctlg").then((r) => {
					this.options.insu = r.data;
				});
			},

			editarInsumo(row) {
				this.ids = row.id;
				this.id_ctlg_insumoss = row.insumos;
				this.formActualizar = true;
			},

			guardar(row) {
				if (
					this.id_ctlg_insumoss !== null &&
					this.id_ctlg_insumoss !== ""
				) {
					_.rput(`insumos/${this.ids}`, null, {
						ids: row.id,
						id_ctlg_insumoss: this.id_ctlg_insumoss,
					})
						.then((r) => {
			...
			eliminar(row) {
				...
				//this.confirm=true;
				this.$q.loading.show();
				_.rdelete(`insumos/elim/${this.ids}`, null, {
					ids: row.id,
				})
					.then((r) => {
						//	this.confirm=false;

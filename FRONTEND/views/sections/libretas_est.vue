	...
	export default {
		data() {
			return {
				...
				exa: 0,
				isHidden: true,
				colors:['bg-indigo', 'bg-teal' , 'bg-purple','bg-pink', 'bg-deep-purple', 'bg-blue', 'bg-green', 'bg-light-blue','bg-cyan', 'bg-lime','bg-yellow','bg-amber','bg-orange','bg-brown','bg-blue-grey', 'bg-red'],
		...
		methods: {
			anos_lectivos(id_estudiantes) {
				return _.rget(`notas/estu_ano/${id_estudiantes}`).then((r) => {
					...
					//console.log(
						//.$root.currentPage.app.informacion.id_estudiantes
					//);
				});
			},
			...
			parciales() {
				this.notas.data = [];
				return _.rget(`parciales/ctlg_par/${this.f._id_periodos}`).then(
					(r) => {

...
app.get("/lista_ctlg_materias", async (req, res) => {
	try {
		var r = await repository.lista_ctlg_materias();
		res.done(r);
	} catch (err) {
		res.fail(400, err);
	}
...
app.post("/ctlg_materias", async (req, res) => {
	try {
		var r = await repository.crear_ctlg_materias(req.body);
		res.done(r);
	} catch (err) {
		res.fail(400, err);
	}
...
app.put("/actualizar_ctlg_materias/:id", async (req, res) => {
	try {
		await validations.id(req.params.id);
		var r = await repository.actualizar_ctlg_materias(
			req.params.id,
			req.body
		);
		res.done(r);
...
app.delete("/eliminar_ctlg_materias/:id", async (req, res) => {
	try {
		await validations.id(req.params.id);
		var r = await repository.eliminar_ctlg_materias(
			req.params.id,
			req.body
		);
		res.done(r);

...
app.get("/materias_anos", async (req, res) => {
	try {
		//await validations.body(req.body);
		var r = await repository.materias_anos(req.query);
		res.done(r);
	} catch (err) {
		res.fail(400, err);
	}
...
app.get("/jornadas_anos", async (req, res) => {
	try {
		//await validations.body(req.body);
		var r = await repository.jornadas_anos(req.query);
		res.done(r);
	} catch (err) {
		res.fail(400, err);
	}
...
app.get("/docentes_materias/:id", async (req, res) => {
	try {
		await validations.id(req.params.id);
		var r = await repository.docentes_materias(req.params.id);
		res.done(r);
	} catch (err) {
		res.fail(400, err);
	}

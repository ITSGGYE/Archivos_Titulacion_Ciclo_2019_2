...
if(isset($_POST["sem"]) && isset($_POST["section"])){
	...
	if($a == "I-I"){
		...
		$count =  mysqli_num_rows($sql);
				...
				<td class='info'><select class='form-control' name='fac'>";
				 $ans = mysqli_query($conexion, "SELECT COUNT(*) AS `Rows`, `name` FROM `faculty` WHERE yr='I-I' GROUP BY `name` ORDER BY `name`");
	...
	// for first year second semester attendance
		...
		$count =  mysqli_num_rows($sql);
				...
				<input type='date' class='form-control' name='date' ></td>";
			  	echo "
				<td class='info'><select class='form-control' name='fac'>";
				 $ans = mysqli_query($conexion, "SELECT COUNT(*) AS `Rows`, `name` FROM `faculty` WHERE yr='I-II' GROUP BY `name` ORDER BY `name`");
				    $count1= mysqli_num_rows($ans);
				while($j<=$count1){
					while($row = mysqli_fetch_array($ans)){
						 $fname = $row["name"];
	...
	// second year first semester attendance
		...
		$count =  mysqli_num_rows($sql);
				...
				<input type='date' class='form-control' name='date' ></td>";
			  	echo "
				<td class='info'><select class='form-control' name='fac'>";
				 $ans = mysqli_query($conexion,"SELECT COUNT(*) AS `Rows`, `name` FROM `faculty` WHERE yr='II-I' GROUP BY `name` ORDER BY `name`");
				    $count1= mysqli_num_rows($ans);
				while($j<=$count1){
	...
	//second year second semester attendacne
		...
		$count =  mysqli_num_rows($sql);
				...
				<input type='date' class='form-control' name='date' ></td>";
			  	echo "
				<td class='info'><select class='form-control' name='fac'>";
				 $ans = mysqli_query($conexion, "SELECT COUNT(*) AS `Rows`, `name` FROM `faculty` WHERE yr='II-II' GROUP BY `name` ORDER BY `name`");
				    $count1= mysqli_num_rows($ans);
				while($j<=$count1){
					while($row = mysqli_fetch_array($ans)){
						 $fname = $row["name"];
	...
	//third year first semester attendance
		...
		$count =  mysqli_num_rows($sql);
				...
				<input type='date' class='form-control' name='date' ></td>";
			  	echo "
				<td class='info'><select class='form-control' name='fac'>";
				 $ans = mysqli_query($conexion,"SELECT COUNT(*) AS `Rows`, `name` FROM `faculty` WHERE yr='III-I' GROUP BY `name` ORDER BY `name`");
				    $count1= mysqli_num_rows($ans);
				while($j<=$count1){
					while($row = mysqli_fetch_array($ans)){
						 $fname = $row["name"];
	...
	// third year second semester aattendance
		...
		$count =  mysqli_num_rows($sql);
				...
				<input type='date' class='form-control' name='date' ></td>";
			  	echo "
				<td class='info'><select class='form-control' name='fac'>";
				 $ans = mysqli_query($conexion, "SELECT COUNT(*) AS `Rows`, `name` FROM `faculty` WHERE yr='III-II' GROUP BY `name` ORDER BY `name`");
				    $count1= mysqli_num_rows($ans);
				while($j<=$count1){
					while($row = mysqli_fetch_array($ans)){
						 $fname = $row["name"];
	...
	//fourth year first semester attendance
		...
		$sql = mysqli_query($conexion, "SELECT id,sem,sec,sname FROM s7 WHERE sec = '$section'");
		$count =  mysqli_num_rows($sql);
					echo "
					<div class='container'><div class='promotess'>
				<form method='post' action='add_attendance.php'>
				...
				<input type='date' class='form-control' name='date' ></td>";
			  	echo "
				<td class='info'><select class='form-control' name='fac'>";
				 $ans = mysqli_query($conexion,"SELECT COUNT(*) AS `Rows`, `name` FROM `faculty` WHERE yr='IV-I' GROUP BY `name` ORDER BY `name`");
				    $count1= mysqli_num_rows($ans);
				while($j<=$count1){
					while($row = mysqli_fetch_array($ans)){
						 $fname = $row["name"];
	...
	//fourth year second semester attendance
		...
		$sql = mysqli_query($conexion, "SELECT id,sem,sec,sname FROM s8 WHERE sec = '$section'");
		$count =  mysqli_num_rows($sql);
					echo "
					<div class='container'><div class='promotess'>
				<form method='post' action='add_attendance.php'>
				...
				<input type='date' class='form-control' name='date' ></td>";
			  	echo "
				<td class='info'><select class='form-control' name='fac'>";
				 $ans = mysqli_query( $conexion, "SELECT COUNT(*) AS `Rows`, `name` FROM `faculty` WHERE yr='IV-II' GROUP BY `name` ORDER BY `name`");
				    $count1= mysqli_num_rows($ans);
				while($j<=$count1){
					while($row = mysqli_fetch_array($ans)){
						 $fname = $row["name"];
...
<?php
	...
	//retriving the period to check whether attendance for that period is inserted within a day or not
	 ...
	 // fourth year first semester attendance
		...
		$check = mysqli_query($conexion,"select id from a7 where (fac='$fac' and day='$date') and (sec = '$sec' and per = '$period') ");
		$countCheck = mysqli_num_rows($check);
//retriving the period to check whether attendance for that period is inserted within a day or not
		$checkSub = mysqli_query($conexion, "select per from a7 where day='$date' and per = '$period' and sec='$sec' ");
		$pers = mysqli_num_rows($checkSub);
		//checking period
		if($pers == $count){
			// attendance for that period is already inserted
		...
		//attendance for that period is not inserted so insert into the database
		if($countCheck != $count){
			$sql1 = mysqli_query($conexion, "INSERT INTO `a7` (`id`, `sem`, `day`, `atten`, `fac`,`sec`,`per`,`sname`) VALUES ('$ides','$sem','$date','$res','$fac','$sec','$period','$names');");
		}
		else{
			echo "<div align='center'><font color='red'><b>Lo siento, la asistencia para este tema ya está insertada
</b></font></div>";
	 ...
	 // fourth year second semenster attendance
		...
		$check = mysqli_query($conexion, "select id from a8 where (fac='$fac' and day='$date') and (sec = '$sec' and per = '$period')  ");
		$countCheck = mysqli_num_rows($check);
	
		//retriving the period to check whether attendance for that period is inserted within a day or not
		$checkSub = mysqli_query($conexion, "select per from a8 where day='$date' and per = '$period' and sec='$sec' ");
		$pers = mysqli_num_rows($checkSub);
		//checking period
		if($pers == $count){
			// attendance for that period is already inserted
		...
		//attendance for that period is not inserted so insert into the database
		if($countCheck != $count){
			$sql1 = mysqli_query($conexion,"INSERT INTO `a8` (`id`, `sem`, `day`, `atten`, `fac`,`sec`,`per`,`sname`) VALUES ('$ides','$sem','$date','$res','$fac','$sec','$period','$names');");
		}
		else{
			echo "<div align='center'><font color='red'><b>Lo siento, la asistencia para este tema ya está insertada
</b></font></div>";

...
<html>
	...
	<body>
		<div class="container" align="center">
			...
			<br/>
			<div class="promote">
			<form method="post" action="add_attendance2.php" >
...
if(isset($_POST["sem"]) && isset($_POST["section"]) && isset($_POST["espe"]) ){
    ...
    if($a == "Primero de Bachillerato"){
				$sql = mysqli_query($conexion,"SELECT id,sem,sec,sname,especializacion FROM s4 WHERE sec = '$section' AND especializacion = '$espc' ");
	...
	//third year first semester attendance
		...
		$sql = mysqli_query($conexion,"SELECT id,sem,sec,sname,especializacion FROM s5 WHERE sec = '$section' AND especializacion = '$espc' ");
		$count =  mysqli_num_rows($sql);
					echo "
					<div class='container'><div class='promotess'>
				<form method='post' action='add_attendance2.php'>
	...
	// third year second semester aattendance
		...
		$sql = mysqli_query($conexion,"SELECT id,sem,sec,sname,especializacion FROM s6 WHERE sec = '$section' AND especializacion = '$espc'");
		$count =  mysqli_num_rows($sql);
					echo "
					<div class='container'><div class='promotess'>
				<form method='post' action='add_attendance2.php'>
...
<?php
	...
	if(isset($_POST["ids$i"]) && isset($_POST["date"]) && (isset($_POST["period"]))){
	 ...
	 if($sem == "Primero de Bachillerato"){
		$check = mysqli_query($conexion,"select id from a4 where (fac='$fac' and day='$date') and (sec = '$sec' and per = '$period' and especializacion='$especi'and especializacion='$especi') ");
	...
	//retriving the period to check whether attendance for that period is inserted within a day or not
		$checkSub = mysqli_query($conexion,"select per from a4 where day='$date' and per = '$period' and sec='$sec' and especializacion='$especi'");
		...
		if($countCheck != $count){
			$sql1 = mysqli_query($conexion,"INSERT INTO `a4` (`id`, `sem`, `day`, `atten`, `fac`,`sec`,`per`,`sname`,`especializacion`) VALUES ('$ides','$sem','$date','$res','$fac','$sec','$period','$names','$especi');");
		}
		else{
	 ...
	 // third year first semester attendance
		...
		$check = mysqli_query($conexion,"select id from a5 where (fac='$fac' and day='$date') and (sec = '$sec' and per = '$period' and especializacion='$especi') ");
		$countCheck = mysqli_num_rows($check);
	//retriving the period to check whether attendance for that period is inserted within a day or not
		$checkSub = mysqli_query($conexion,"select per from a5 where day='$date' and per = '$period' and sec='$sec' and especializacion='$especi' ");
		$pers = mysqli_num_rows($checkSub);
		//checking period
		if($pers == $count){
			// attendance for that period is already inserted
		...
		//attendance for that period is not inserted so insert into the database
		if($countCheck != $count){
			$sql1 = mysqli_query($conexion,"INSERT INTO `a5` (`id`, `sem`, `day`, `atten`, `fac`,`sec`,`per`,`sname`,`especializacion`) VALUES ('$ides','$sem','$date','$res','$fac','$sec','$period','$names','$especi');");
		}
		else{
			echo "<div align='center'><font color='red'><b>Lo siento, la asistencia para este período ya está insertada
</b></font></div>";
	 ...
	 // third year second semester attendance
		...
		$check = mysqli_query($conexion,"select id from a6 where (fac='$fac' and day='$date') and (sec = '$sec' and per = '$period' and especializacion='$especi'and especializacion='$especi') ");
		$countCheck = mysqli_num_rows($check);
	//retriving the period to check whether attendance for that period is inserted within a day or not
		$checkSub = mysqli_query($conexion,"select per from a6 where day='$date' and per = '$period' and sec='$sec' and especializacion='$especi'");
		$pers = mysqli_num_rows($checkSub);
		//checking period
		if($pers == $count){
			// attendance for that period is already inserted
		...
		//attendance for that period is not inserted so insert into the database
		if($countCheck != $count){
			$sql1 = mysqli_query($conexion,"INSERT INTO `a6` (`id`, `sem`, `day`, `atten`, `fac`,`sec`,`per`,`sname`,`especializacion`) VALUES ('$ides','$sem','$date','$res','$fac','$sec','$period','$names','$especi');");
		}
		else{
			echo "<div align='center'><font color='red'><b>Lo siento, la asistencia para este período ya está insertada
</b></font></div>";

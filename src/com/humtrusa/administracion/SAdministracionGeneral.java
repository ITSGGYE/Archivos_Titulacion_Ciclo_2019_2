...
public class SAdministracionGeneral extends HttpServlet {
	...
	public String obtenerAgenciasxEmpresa(HttpServletRequest request){
		...
		Genusuarios usu = (Genusuarios)sesion.getAttribute("session");
		
		long codempresa = Long.parseLong(request.getParameter("codEmpresa")); 
		long codestado = Long.parseLong(request.getParameter("estado"));
		String user = usu.getCodusuario();
		try {		
		str= beanGeneral.obtenerAgenciasxEmpresa(codempresa,codestado,user);
		}catch(Exception e) {
			System.out.println("EROOR ");e.printStackTrace();
		}
		return str;
	...
	public String listarEstados(HttpServletRequest request){
		System.out.println("listarEstados");
		long codigoempresa = Long.parseLong(request.getParameter("codempresa"));
		String codestados = (request.getParameter("codestados")!=null && !request.getParameter("codestados").replace(" ","").equals(""))?request.getParameter("codestados"):null;
				
		return beanGeneral.obtenerListaEstados(codigoempresa, codestados);
	}
	...
	private void procesar(HttpServletRequest request,
		...
		switch(enumOrdenes){
			...
			case OBTENER_ESTADOS:
				out.print(listarEstados(request));
				break;
		}

var Plantillas = function() {
	...
	var botonesToolbar={
		...
		cmdVerPDF : new Ext.Toolbar.Button({
						text : "Visualizar PDF Resumido",
						icon : "../../imagenes/pdf.png",
						cls : "x-btn-text-icon"
					}),
		
		cmdExportarEXCEL : new Ext.Toolbar.Button({
					text : "Exportar EXCEL Resumido",
					icon : "../../imagenes/EXCEL.png",
					cls : "x-btn-text-icon"
				}),
				
		cmdVerPDFD : new Ext.Toolbar.Button({
						text : "Visualizar PDF Detallado",
						icon : "../../imagenes/pdf.png",
						cls : "x-btn-text-icon"
					}),
		
		cmdExportarEXCELD : new Ext.Toolbar.Button({
					text : "Exportar EXCEL Detallado",
					icon : "../../imagenes/EXCEL.png",
					cls : "x-btn-text-icon"
				})
	...
	var panelCentro = new Ext.grid.GridPanel({
		...
		tbar : new Ext.Toolbar({
			items : [{xtype : "tbseparator"}, 
					 botonesToolbar.cmdVerPDF,
					 botonesToolbar.cmdExportarEXCEL,
					 botonesToolbar.cmdVerPDFD,
					 botonesToolbar.cmdExportarEXCELD
					]
		}),
	...
	*/
	...
	function preparaParametrosD(){
		fdesde = formatDate(componentesFiltro.txtFechaDesde.getValue());
		fhasta = formatDate(componentesFiltro.txtFechaHasta.getValue());
		//alert(fdesde+" "+fhasta);
		var parametrosReporte = [
			...
			{key:"fechadesde",value: fdesde},
			{key:"fechaHasta",value: fhasta}
		];
		
		reportePromocion = new CreadorReportes("REPORTE_COMPRAS_DETALLADO",null);
	...
	function preparaParametros(){
		...
		reportePromocion = new CreadorReportes("REPORTE_COMPRAS_RESUMEN",null);	
		reportePromocion.modificarParametros(parametrosReporte);		
	};		
	...
	var visualizarPDFD = function (){
		preparaParametrosD(); 
		var tiraOrden = reportePromocion.obtenerTiraReporte(TiposExportacion.VER_PDF);
		window.open(tiraOrden);
	};
	
	var imprimirXLSD = function (){
		preparaParametrosD();			
		var tiraOrden = reportePromocion.obtenerTiraReporte(TiposExportacion.XLS);
		window.open(tiraOrden);
	};
	
	botonesToolbar.cmdVerPDF.on("click",function(){
		visualizarPDF();
	});
	botonesToolbar.cmdExportarEXCEL.on("click",function(){
		imprimirXLS();
	});
	botonesToolbar.cmdVerPDFD.on("click",function(){
		visualizarPDFD();
	});
	botonesToolbar.cmdExportarEXCELD.on("click",function(){
		imprimirXLSD();
	});
};

var PlantillasProceso = function() {
	...
	var storeDatos = {
		...
		storeComboBodegasxAgencia : new Ext.data.Store({
	...
	var componentes = {
		...
		cmbBodegasxAgenciasG : new Ext.form.ComboBox({
					fieldLabel : "Bodega",
					allowBlank : false,
					name : "cmbBodegasxAgenciasG",
					emptyText : "Bodega Destino General",
					store : storeDatos.storeComboBodegasxAgencia,
					triggerAction : "all",
					mode : "remote",
					width : 180,
					typeAhead : false,
	...
	var checkColumVerificado = new Ext.grid.CheckColumn({
	...
	var modelosDatos = {
		storeComboesPrincipal: new Ext.data.SimpleStore({
			fields:["codigo","descripcion"],
			data:[["S","SI"],["N","NO"]]
		}),
		storeComboBodega :new Ext.data.SimpleStore({
			fields : ["codigo", "descripcion"],
			data : [["1", "CEDULA"], ["2", "RUC"],
					["3", "PASAPORTE"]]
		})
	...
	var PanelgridDatos = new Ext.Panel({
		...
		items:[
				...
				border : false,
				loadMask : true,
				//plugins : [checkColumnCabecera, checkColumVerificado],
						...
						{dataIndex : "codarticulo",name : "codarticulo",header : "codarticulo",width : 100,hidden : true,hideable : true},
						{
							...
							editor : new Ext.form.ChooserField({
										...
										onTriggerClick : function() {
											//abrirAgrupador(this, "A");
											//alert("on tigger");
											if(componentes.cmbEstadoPedido.getValue() != 7)
												AgrupadorArticulo();
										},
										listeners : {
											scope : this,
							...
							width : 90, 
							align:'right',
							//editor : new Ext.form.TextField({maskRe:/[0-9.]/,value:0.00,selectOnFocus:true}),
							...
							width : 90,
							align:'right',
							editor : new Ext.form.TextField({maskRe:/[0-9.]/,value:"",selectOnFocus:true
															/*listeners:{'specialkey':function(e){if((e.value*1) <= 0.00){
															var celda = Ext.getCmp('gridDatos').getSelectionModel().getSelectedCell();
															Ext.getCmp('gridDatos').startEditing(celda[0],celda[1]);return;}}}*/
							}),
							renderer : Ext.util.Format.monedaSinSimbolo
						},{ 
							dataIndex : 'canttotal',
							...
							width : 90, 
							align:'right',
							//editor : new Ext.form.TextField({maskRe:/[0-9.]/,selectOnFocus:true}),
							renderer : Ext.util.Format.monedaSinSimbolo
						},{
							dataIndex : 'precio',
							type : 'string',
				...
				tbar : new Ext.Toolbar({
							items : [{
										...
										componentes.cmbBodegasxAgenciasG
									]
						})
	...
	panelSurTotales = new Ext.Panel({
		...
		html : '<div align="right" style="width:50%;float:right">'
				...
				+'<div align="left" style="width:50%;">'
  					+'<table border="0">'
						+'<tr>'
							+'<label class="lbl_totales" for="fname">OBSERVACI&Oacute;N:  </label><textarea id="textAreaObservacion" style="width:350px;height:50px;background: #FFFFCC !important;border:1px solid black !important;" id="fname" rows="2" cols="50" width="150px" placeholder="Ingrese una Observaci&oacute;n..."></textarea><br>' 
						+'</tr>'
						+'</table>'
				+'</div>'
	...
	var panelPrincipal = new Ext.form.FormPanel({
		...
		buttons : [ botones.cmdExportarEXCEL, botones.cmdExportarPDF, botones.cmdVerPDF, botones.cmdGuardar,botones.cmdCancelar]
	});
	...
	var prepararVentana = function() {
		...
		//Ext.getCmp("gridDatos").getView().getColumnModel(4).style.backgroundColor = 'LightGreen';
	...
	var cargarRegistro = function(data) {
		...
		document.getElementById("textAreaObservacion").value = data.observacion;
	...
	*/
	...
	function calcularValoresDetalle(record){
		...
		var cantidad = (record.get("cantrecibida")<0?record.get("cantsolicitada"):record.get("cantrecibida")) || 0;
		...
		var cantrecibida = record.get("cantrecibida");
		...
		record.set("canttotal",cantrecibida);
	...
	Ext.getCmp("gridDatos").on('afteredit',function(e){
		...
		//alert("afteredit");	
	 	...
	 	if((e.record.get('cantrecibida')*1)<(e.record.get('cantsolicitada')*1)){
	 		Ext.getCmp("gridDatos").getView().getRow(e.row).style.backgroundColor = 'LightBlue';
	 	}else{
	 		Ext.getCmp("gridDatos").getView().getRow(e.row).style.backgroundColor = 'LightYellow';
	 	}
	});
	...
	Ext.getCmp("gridDatos").on('validateedit',function(e){
		...
		switch(e.field){
				...
				case 'cantrecibida':
					//if(e.value == 0 || e.value == ''){ return false; }
					e.value = e.value || '0';
					if((e.value*1) > (e.record.get('cantsolicitada'))){
						Ext.MessageBox.show({ 
							title: 'Atención',
							msg: 'La Cantidad Recibida no puede ser mayor a la Cantidad Solicitada.',
							buttons: Ext.MessageBox.OK,
							scope:this,
							fn:function(){
								var celda = gridDatos.getSelectionModel().getSelectedCell();					
					...
					if((e.value*1) <= 0.00){
						Ext.MessageBox.show({ 
							title: 'Atención',
							msg: 'La Cantidad Recibida no puede ser menor a 0.',
							buttons: Ext.MessageBox.OK,
							scope:this,
							fn:function(){
								var celda = gridDatos.getSelectionModel().getSelectedCell();					
		...
		Ext.getCmp("gridDatos").on("render", function () {
			//alert("render");
				var gridDatos = Ext.getCmp("gridDatos");
				var mapGridDetalle = new Ext.KeyMap(Ext.getCmp("gridDatos").body,[
					{ 
		...
		PanelgridDatos.on("blur", function (e) { 
			alert("click "); 
		});
	...
	var insertGridArt = function (data){
			...
			record.set("cantrecibida", "");
			record.set("canttotal", '');
	...
	componentes.cmbBodegasxAgenciasG.on("select",function( combo, record, index){
		alert(combo.getValue());
		setearComboBodegaGrl(combo.getValue());
	});
	
	var setearComboBodegaGrl = function(idCombo){
		var gridDatos = Ext.getCmp("gridDatos");
		var store = gridDatos.getStore();
				
		store.each(function(record,e){
			//alert(record.get('verificado'));
			if(record.get('codarticulo')){
				record.set('bodega',idCombo)
			}
		});
	
	};
	
	var validarRecepcionCompras = function(){
		var gridDatos = Ext.getCmp("gridDatos");
		var store = gridDatos.getStore();
		var mensajecantidad = "Los articulos {2} No pueden Tener Cantidad menor a 0.";
		var mensajebodega = "Los articulos {0} necesitan que se les asignen una Bodega.";
		var mensajeVerificado = "Los articulos {1} necesitan ser Verificados (Check).";
		var articulosa = ""; var articulosb = "";  var articulosC = ""; var cadena ="";
		var i=0;
		store.each(function(record){
			if(record.get('codarticulo')){
				if(record.get('cantrecibida') == undefined || (record.get('cantrecibida')*1) <= 0.00){
					articulosa += ""+(i>0?",":"")+record.get('codalterno')+"";
				}
				if(record.get('bodega') == undefined || record.get('bodega') <= 0){
					articulosb += ""+(i>0?",":"")+record.get('codalterno')+"";
				}
				if(record.get('verificado') == undefined ||record.get('verificado') == false){
					articulosC += ""+(i>0?",":"")+record.get('codalterno')+"";
				}
			}
			i++;
		});
		...
		if(articulosb != ""){
			cadena += "<br>"; 
			cadena += mensajebodega.replace("{0}",articulosb);
		}
		
		if(articulosC != ""){
			cadena += "<br>"; 
			cadena += mensajeVerificado.replace("{1}",articulosC);
		}
		...
		if(cadena != ""){
			...
			var observ = document.getElementById("textAreaObservacion").value;
			if(observ==null || observ==""){
				Ext.Msg.show({
							title : "Alerta",
							msg : "Ingrese una Observaci&oacute;n",
							icon : Ext.Msg.ERROR,
							buttons : Ext.Msg.OK,
							fn:function(){
								return;
	...
	/**
	 * janchundia reportes 1.0
	 */
	var reportePromocion = null;
	/*
	function preparaParametros(){
		var parametrosReporte = [
		    {key:"EMPRESA_LOGONEADA",value: document.parametrosSesion.codigoEmpresa},
			{key:"AGENCIA_LOGONEADA",value: document.parametrosSesion.codigoAgencia},
			{key:"codigo",value: 3}
		];
		reportePromocion = new CreadorReportes("REPORTE_PRUEBA",null);
	...
	*/
	function preparaParametros(){
		var parametrosReporte = [
			...
			{key:"numdocumento",value: ""},
			{key:"codestado",value: componentes.cmbEstadoPedido.getValue()}
		];
		reportePromocion = new CreadorReportes("REPORTE_COMPRAS_PROCESADO",null);
		reportePromocion.modificarParametros(parametrosReporte);		
	};
	...
	function imagenImpimirXLS(){	    		
		return '<a href=\'#\' onclick=\'document.programa.imprimirXLS();\' ><img src=\'./../../imagenes/page_excel.png\' style=\'border:0\' alt=\'Ver Stock en Bodega\' title=\'Imprimir XLS\'/></a>';
	}
	
	function imagenImpimirPDF(){	    
		return '<a href=\'#\' onclick=\'document.programa.imprimirPDF();\' ><img src=\'./../../imagenes/pdf.png\' style=\'border:0\' alt=\'Ver Stock en Bodega\' title=\'Imprimir PDF\'/></a>';	
	}
	
	var efectuarCompra  = function(){
		var paramsGuardar = {
			url : "../../servlet/SAdministracionPedidoCompra",
			params : {
					//CABECERA COMPRA
					orden : "GUARDAR_COMPRA",
					codEmpresa : document.parametrosSesion.codigoEmpresa,
			        codagenciapedido : document.parametrosSesion.codigoAgencia,
			        numerocompra: componentes.txtnombres.getValue(),
			        codestado: componentes.cmbEstadoPedido.getValue(),			     
			        codproveedor: componentes.tfcodProveedor.getValue(),
			        codusuario: document.parametrosSesion.codigoUsuario,
			        fecha: componentes.txtDireccion.getRawValue(),
			        observacion: document.getElementById("textAreaObservacion").value,
			        subtotal: document.getElementById("subTotalGeneral").innerHTML,
			        descuento: document.getElementById("descuentoGeneral").innerHTML,
			        porcimpuesto: 12, //
			        impuesto: document.getElementById("impuestoGeneral").innerHTML,
			        total : document.getElementById("totalGeneral").innerHTML,
			        //Detalle COMPRA
			        detalleCompra: obtenerJSONGrid("gridDatos")
				},
			callback : function(option, success, response) {
				var respuesta = Ext.decode(response.responseText);

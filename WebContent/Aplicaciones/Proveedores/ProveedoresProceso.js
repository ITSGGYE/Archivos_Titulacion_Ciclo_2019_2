var PlantillasProceso = function() {
	...
	var componentes = {
		...
		txtidentificacion : new Ext.form.TextField({
					...
					required: true,
					validateOnBlur : true,
					blankText:'Ingresa una Identificaci&oacute;n',
					maskRe : /[\d\.]/,
					regex : /^\d+(\.\d{1,2})?$/,
		...
		txtobservacion: new Ext.form.TextArea({
		...
		txtcargocontacto : new Ext.form.TextField({
					fieldLabel : "Carg. Contacto",
					name : "txtcargocontacto",
					readOnly : false,
					width : 164,
					maskRe : /[A-Za-z. ]/,
					minLength: 5,
	...
	var panelSecundario = new Ext.Panel({
				...
				items : [new Ext.Panel({
							...
							items : [
									...
									componentes.txtcargocontacto,
									componentes.txttelefonoFijo2,
									componentes.txtobservacion]
						})
	...
	var guardado = function() {
			...
			if("I" == configuraciones.modoActual){
				 accion={
					...
					cargocontacto : componentes.txtcargocontacto.getValue(),
					contacto : componentes.txtcontacto.getValue(),
					observacion : componentes.txtobservacion.getValue(),
					direccion : componentes.txtDireccion.getValue(),
			...
			callback : function(option, success, response) {
				...
				if (success) {
					if (respuesta.exito) {
						...
						componentes.txtidentificacion.setValue(respuesta.identificacion);
						componentes.txtDireccion.setValue(respuesta.direccion);
						componentes.txttelefonoCelular.setValue(respuesta.celular);
						componentes.txttelefonoFijo1.setValue(respuesta.tfijo1);
						componentes.txttelefonoFijo2.setValue(respuesta.tfijo2);
						componentes.cmbtipoidentificacion.setValue(respuesta.tidentificacion);
						componentes.cmbesTransportista.setValue(respuesta.estransportista);
						componentes.txtEmail.setValue(respuesta.email);
						componentes.txtFax.setValue(respuesta.fax);
						componentes.txtcontacto.setValue(respuesta.contacto);
						componentes.txtcargocontacto.setValue(respuesta.cgcontacto);
						componentes.txtobservacion.setValue(respuesta.observacion);

...
/**
 * Busqueda por codigo Retorna codigo y descripcion 
 * @param {} configuracion
 * @author Janchundia
 */
var Busquedaxcodigo = function (configuracion){
	...
	var llamarAjax = function(){
		var parametros = {
			url : ParametrosGenerales.url,
			params : {
				orden : ParametrosGenerales.orden,
				codempresa : document.parametrosSesion.codigoEmpresa,
				codigo : ParametrosGenerales.codigoB,
				tiporespuesta : "JSON"
			},
			callback : function(option, success, response) {
				...
				var obj = null;
				if (success) {
					//alert(respuesta.exito+ " "+Ext.encode(respuesta.mensaje));
					if (respuesta.exito) {
						if(respuesta.localizo){
							obj= Ext.encode(respuesta.mensaje);
							devolverRespuesta(Ext.decode(obj));
						}else{
							devolverRespuesta(null);
						}
	...
	function devolverRespuesta(registro){
		ParametrosGenerales.apuntador.getDataSelection(registro);
	};
	
	this.cargarFiltroCodigo = function(){
		llamarAjax();
	};

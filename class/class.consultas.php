...
class Json
{
    ...
    $consulta = "SELECT 
    CONCAT(productos.codproducto, ' | ',productos.producto, ' | ',categorias.nomcategoria, '') as label, productos.codproducto, productos.producto, productos.codcategoria, productos.preciocompra, productos.precioventa, productos.existencia, productos.ivaproducto, productos.descproducto, categorias.nomcategoria FROM productos INNER JOIN categorias ON productos.codcategoria=categorias.codcategoria WHERE CONCAT(codproducto, '',producto, '',nomcategoria, '',codigobarra) LIKE '%".$filtro."%' GROUP BY codproducto ASC LIMIT 0,15";
    ...
    $consulta = "SELECT 
    CONCAT(insumos.codinsumo, ' | ',insumos.insumo, ' | ',categorias.nomcategoria, '') as label, insumos.codinsumo, insumos.insumo, insumos.codcategoria, insumos.preciocompra, insumos.precioventa, insumos.existencia, insumos.stockminimo, insumos.ivainsumo, insumos.descinsumo, insumos.fechaexpiracion, insumos.codproveedor, categorias.nomcategoria FROM insumos INNER JOIN categorias ON insumos.codcategoria=categorias.codcategoria WHERE CONCAT(codinsumo, '',insumo, '',nomcategoria) LIKE '%".$filtro."%' GROUP BY codinsumo ASC LIMIT 0,15";
        $conexion = new conectorDB;
		$this->json = $conexion->EjecutarSentencia($consulta);
		return $this->json;
	}

...
/*https://www.getintopc.com/?fbclid=IwAR23iuTS1338KAdoh-f2wi8B42CPvEGq9_tEPhP1BU6iO81Xffny8QP3-qU*/
############################### CLASE LOGIN  ################################
...
##################### FUNCION PARA EXPIRAR SESSION POR INACTIVIDAD ####################
	...
	$inactividad =36000000000000000000000000000000; //600 equivale a 5 minutos
...
public function EliminarDocumentos()
{
	...
	if ($_SESSION['acceso'] == "administrador") {
		...
		$sql = "SELECT coddocumento FROM clientes WHERE documcliente = ? OR proveedores WHERE documproveedor";
		$stmt = $this->dbh->prepare($sql);
		$stmt->execute(array(decrypt($_GET["coddocumento"])));
		$num = $stmt->rowCount();
		if($num == 0)
...
public function EliminarMediosPagos()
{
	self::SetNames();
		...
		$sql = "SELECT formapagov FROM ventas WHERE formapagov = ?";
		$stmt = $this->dbh->prepare($sql);
		$stmt->execute(array(decrypt($_GET["codmediopago"])));
		$num = $stmt->rowCount();
		if($num == 0)
...
public function EliminarTipos()
{
	...
	if($_SESSION['acceso'] == "administrador") {
		...
		$sql = "SELECT codtipo FROM habitaciones WHERE codtipo = ?";
		$stmt = $this->dbh->prepare($sql);
		$stmt->execute(array(decrypt($_GET["codtipo"])));
		$num = $stmt->rowCount();
		if($num == 0)
...
public function RegistrarTarifas()
{
	...
	if(empty($_POST["codtipo"]) or empty($_POST["baja"]) or empty($_POST["media"]) or empty($_POST["alta"]))
	{
	...
	else if($_POST["baja"]=="0.00" or $_POST["media"]=="0.00" or $_POST["alta"]=="0.00")
		{
			...
			$sql = " SELECT codtipo FROM tarifas WHERE nomtipo = ?";
			$stmt = $this->dbh->prepare($sql);
...
############################ FUNCION REGISTRAR HABITACIONES ###########################
	...
	{
		self::SetNames();
		if(empty($_POST["numhabitacion"]) or empty($_POST["descriphabitacion"]) or empty($_POST["piso"]) or empty($_POST["vista"]) or empty($_POST["maxadultos"]) or empty($_POST["maxninos"]) or empty($_POST["codtarifa"]))
		{
			echo "1";
			exit;
		}
...
############################# FUNCION LISTAR HABITACIONES #############################
	...
	{
		self::SetNames();
		$sql = "SELECT * FROM habitaciones INNER JOIN tarifas ON habitaciones.codtarifa = tarifas.codtarifa INNER JOIN tiposhabitaciones ON tarifas.codtipo = tiposhabitaciones.codtipo ORDER BY habitaciones.codhabitacion ASC";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
################### FUNCION LISTAR HABITACIONES AGRUPADAS POR TIPOS #################
	...
	{
		self::SetNames();
		$sql = "SELECT * FROM tiposhabitaciones LEFT JOIN tarifas ON tiposhabitaciones.codtipo = tarifas.codtipo LEFT JOIN habitaciones ON tarifas.codtarifa = habitaciones.codtarifa GROUP BY tiposhabitaciones.codtipo ASC";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
############################ FUNCION ACTUALIZAR HABITACIONES ############################
	...
	self::SetNames();
		if(empty($_POST["codhabitacion"]) or empty($_POST["numhabitacion"]) or empty($_POST["descriphabitacion"]) or empty($_POST["piso"]) or empty($_POST["vista"]) or empty($_POST["maxadultos"]) or empty($_POST["maxninos"]) or empty($_POST["codtarifa"]))
		{
			echo "1";
			exit;
		}
...
########################### FUNCION REGISTRAR DE INSUMOS ###############################
	...
	{
		self::SetNames();
		if(empty($_POST["insumo"]) or empty($_POST["codcategoria"]) or empty($_POST["existencia"]))
		{
			echo "1";
			exit;
		}

		$sql = "SELECT codinsumo FROM insumos ORDER BY idinsumo DESC LIMIT 1";
		foreach ($this->dbh->query($sql) as $row){
		...
		$sql = " SELECT insumo FROM insumos WHERE insumo = ? AND codcategoria = ?";
		$stmt = $this->dbh->prepare($sql);
		$stmt->execute(array($_POST["insumo"],$_POST["codcategoria"]));
		$num = $stmt->rowCount();
		if($num == 0)
		{
			$query = " INSERT INTO insumos values (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
...
########################### FUNCION LISTAR DE INSUMOS ################################
	...
	{
		self::SetNames();
	    $sql = "SELECT insumos.idinsumo, insumos.codinsumo, insumos.insumo, insumos.codcategoria, insumos.preciocompra, insumos.precioventa, insumos.existencia, insumos.stockminimo, insumos.ivainsumo, insumos.descinsumo, insumos.fechaexpiracion, insumos.codproveedor, insumos.lote, categorias.nomcategoria, proveedores.cuitproveedor, proveedores.nomproveedor FROM insumos LEFT JOIN categorias ON insumos.codcategoria = categorias.codcategoria LEFT JOIN proveedores ON insumos.codproveedor = proveedores.codproveedor";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
############################ FUNCION ACTUALIZAR DE INSUMOS ############################
	...
	self::SetNames();
		...
		$stmt = $this->dbh->prepare($sql);
		$stmt->execute(array($_POST["codinsumo"],$_POST["insumo"],$_POST["codcategoria"]));
		$num = $stmt->rowCount();
		if($num == 0)
		{
...
public function BuscarkardexInsumo() 
		...
		echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<center><span class='fa fa-info-circle'></span> NO EXISTEN MOVIMIENTOS EN kardex_productos PARA EL INSUMO INGRESADO</center>";
		echo "</div>";		
		exit;
		}
		else
...
public function BuscarkardexProducto() 
		...
		echo "<div class='alert alert-danger'>";
		echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<center><span class='fa fa-info-circle'></span> NO EXISTEN MOVIMIENTOS EN kardex_productos PARA EL PRODUCTO INGRESADO</center>";
		echo "</div>";		
		exit;
		}
		else
...
if (limpiar($_POST["tipocompra"]=="CONTADO")) { $statuscompra = limpiar("PAGADA"); } else { $statuscompra = "PENDIENTE"; }
		...
		$stmt->bindParam(11, $descproductoc);
		$stmt->bindParam(12, $descfactura);
		$stmt->bindParam(13, $valortotal);
		$stmt->bindParam(14, $totaldescuentoc);
...
if (limpiar($detalle[$i]['fechaexpiracion']=="")) { $fechaexpiracionc = "0000-00-00";  } else { $fechaexpiracionc = limpiar(date("Y-m-d",strtotime($detalle[$i]['fechaexpiracion']))); }
		...
		$existencia = limpiar($detalle[$i]['cantidad']+$existenciaproductobd);
...
if (isset($_FILES['sel_file']['size'])) { $tamano_archivo = limpiar($_FILES['sel_file']['size']); } else { $tamano_archivo =''; } 
		...
		$stmt->bindParam(13, $tipopago);
		$stmt->bindParam(14, $montopagado);
		$stmt->bindParam(15, $montodevuelto);
		$stmt->bindParam(16, $tipotarjeta);
		$stmt->bindParam(17, $nrotarjeta);
		$stmt->bindParam(18, $expira);
		$stmt->bindParam(19, $codverifica);
		$stmt->bindParam(20, $nrotransferencia);
		$stmt->bindParam(21, $cheque);
		$stmt->bindParam(22, $bancocheque);
		$stmt->bindParam(23, $fechavencecredito);
		$stmt->bindParam(24, $fechapagado);
		$stmt->bindParam(25, $observaciones);
		$stmt->bindParam(26, $statuspago);
		$stmt->bindParam(27, $reservacion);
		$stmt->bindParam(28, $fecharegistro);
		$stmt->bindParam(29, $codigo);
	    
		$tipodocumento = limpiar($_POST["tipodocumento"]);
		$codcaja = (limpiar(decrypt($_POST["tipo"]))=="1" ? $caja_abierta : "0");
		$codcliente = limpiar($_POST["codcliente"]);
	...
	if (limpiar(isset($_POST['tipotarjeta']))) { $tipotarjeta = limpiar($_POST['tipotarjeta']); } else { $tipotarjeta = limpiar("0"); }
	if (limpiar(isset($_POST['nrotarjeta']))) { $nrotarjeta = limpiar($_POST['nrotarjeta']); } else { $nrotarjeta = limpiar("0"); }
	if (limpiar(isset($_POST['expira']))) { $expira = limpiar($_POST['expira']); } else { $expira = limpiar("0"); }
	if (limpiar(isset($_POST['codverifica']))) { $codverifica = limpiar($_POST['codverifica']); } else { $codverifica = limpiar("0"); }
    if (limpiar(isset($_POST['nrotransferencia']))) { $nrotransferencia = limpiar($_POST['nrotransferencia']); } else { $nrotransferencia = limpiar("0"); }
	if (limpiar(isset($_POST['cheque']))) { $cheque = limpiar($_POST['cheque']); } else { $cheque = limpiar("0"); }
	if (limpiar(isset($_POST['bancocheque']))) { $bancocheque = limpiar($_POST['bancocheque']); } else { $bancocheque = limpiar("0"); }

	if (limpiar(isset($_POST['fechavencecredito']))) { $fechavencecredito = limpiar(date("Y-m-d",strtotime($_POST['fechavencecredito']))); } else { $fechavencecredito = limpiar("0000-00-00"); }
	    $fechapagado = limpiar("0000-00-00");
    if (limpiar(isset($_POST['observaciones']) and $_POST["observaciones"]!="")) { $observaciones = limpiar($_POST['observaciones']); } else { $observaciones = limpiar("0"); }
	if (limpiar($_POST["tipopago"]=="CREDITO")) { $statuspago = limpiar("PENDIENTE"); } else { $statuspago = limpiar("PAGADA"); }
        ...
        $codigo = (limpiar(decrypt($_POST["tipo"]))=="1" ? limpiar($_SESSION["codigo"]) : limpiar("0"));
		$stmt->execute();
    ...
    ############ AGREGAMOS EL INGRESO DE RESERVACIONES PAGADAS A CAJA ##########
	if (limpiar($_POST["tipopago"]=="EFECTIVO") && limpiar(decrypt($_POST["tipo"]))=="1"){

		$sql = "SELECT ingresos FROM arqueocaja WHERE codcaja = '".limpiar($caja_abierta)."' AND statusarqueo = '1'";
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
    ...
    ######### AGREGAMOS EL INGRESO Y ABONOS DE RESERVACIONES A CREDITOS A CAJA ##########
if (limpiar($_POST["tipopago"])=="CREDITO" && limpiar(decrypt($_POST["tipo"]))=="1" && limpiar($_POST["montoabono"])=="0.00" && limpiar($_POST["montoabono"])=="0") {

		$sql = "SELECT creditos FROM arqueocaja WHERE codcaja = '".limpiar($caja_abierta)."' AND statusarqueo = '1'";
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
...
} else if (limpiar($_POST["tipopago"])=="CREDITO" && limpiar(decrypt($_POST["tipo"]))=="1" && limpiar($_POST["montoabono"])!="0.00" && limpiar($_POST["montoabono"])!="0") { 

		$sql = "SELECT creditos, abonos FROM arqueocaja WHERE codcaja = '".limpiar($caja_abierta)."' AND statusarqueo = '1'";
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
...
public function ListarReservaciones()
{
	...
	reservaciones.nrotransferencia,  
...
#################### FUNCION BUSQUEDA RESERVACIONES POR CAJAS #####################
	...
	LEFT JOIN clientes ON reservaciones.codcliente = clientes.codcliente
	LEFT JOIN documentos ON clientes.documcliente = documentos.coddocumento
	LEFT JOIN usuarios ON reservaciones.codigo = usuarios.codigo WHERE reservaciones.codcaja = ? AND DATE_FORMAT(reservaciones.fecharegistro,'%Y-%m-%d') >= ? AND DATE_FORMAT(reservaciones.fecharegistro,'%Y-%m-%d') <= ? GROUP BY detallereservaciones.codreservacion";
		$stmt = $this->dbh->prepare($sql);
		$stmt->bindValue(1, trim(decrypt($_GET['codcaja'])));
		$stmt->bindValue(2, trim(date("Y-m-d",strtotime($_GET['desde']))));
		$stmt->bindValue(3, trim(date("Y-m-d",strtotime($_GET['hasta']))));
...
###################### FUNCION BUSQUEDA RESERVACIONES POR FECHAS #####################
	...
	LEFT JOIN clientes ON reservaciones.codcliente = clientes.codcliente
	LEFT JOIN documentos ON clientes.documcliente = documentos.coddocumento
	LEFT JOIN usuarios ON reservaciones.codigo = usuarios.codigo WHERE DATE_FORMAT(detallereservaciones.desde,'%Y-%m-%d') >= ? AND DATE_FORMAT(detallereservaciones.hasta,'%Y-%m-%d') <= ? GROUP BY detallereservaciones.codreservacion";
		$stmt = $this->dbh->prepare($sql);
		$stmt->bindValue(1, trim(date("Y-m-d",strtotime($_GET['desde']))));
		$stmt->bindValue(2, trim(date("Y-m-d",strtotime($_GET['hasta']))));
		$stmt->execute();
...
###################### FUNCION BUSQUEDA RESERVACIONES POR CLIENTES #####################
	...
	LEFT JOIN documentos ON clientes.documcliente = documentos.coddocumento
	WHERE reservaciones.codcliente = ? GROUP BY detallereservaciones.codreservacion";
		$stmt = $this->dbh->prepare($sql);
...
########################### FUNCION REGISTRAR VENTAS ############################
	public function RegistrarVentas()
	{
		...
		$stmt->bindParam(15, $tipopago);
		$stmt->bindParam(16, $formapago);
		$stmt->bindParam(17, $montopagado);
		$stmt->bindParam(18, $montodevuelto);
		$stmt->bindParam(19, $fechavencecredito);
		$stmt->bindParam(20, $fechapagado);
		$stmt->bindParam(21, $statusventa);
		$stmt->bindParam(22, $fechaventa);
...
public function ActualizarVentas()
	{
	self::SetNames();
	if(empty($_POST["codventa"]) or empty($_POST["tipopago"]))
	{
		echo "1";
		exit;
	}
...
public function AgregarDetallesVentas()
	{
	   ...
	   if ($_POST["tipopago"] == "CREDITO") {
		...
		$sql3 = " UPDATE kardex_productos set "
		      ...
		      ." stockactual = ? "
			  ." WHERE "
			  ." codproceso = '".limpiar($_POST["codventa"])."' and codproducto = '".limpiar($detalle[$i]['txtCodigo'])."';
			   ";
		$stmt = $this->dbh->prepare($sql3);
		$stmt->bindParam(1, $salidas);
		$stmt->bindParam(2, $existencia);
...
public function EliminarDetallesVentas()
{
	...
	if ($_SESSION["acceso"]=="administrador") {
        ...
        ############ CONSULTO TOTAL ACTUAL ##############
		$sql = "SELECT codcaja, codcliente, tipopago, totalpago FROM ventas WHERE codventa = '".limpiar(decrypt($_GET["codventa"]))."'";
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
...
public function BuscarVentasDiarias()
{
	...
	LEFT JOIN documentos AS documentos ON clientes.documcliente = documentos.coddocumento
	LEFT JOIN mediospagos ON ventas.formapago = mediospagos.codmediopago 
	LEFT JOIN usuarios ON ventas.codigo = usuarios.codigo WHERE DATE_FORMAT(ventas.fechaventa,'%d-%m-%Y') = '".date("d-m-Y")."' GROUP BY detalleventas.codventa";
	foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
	...
	LEFT JOIN documentos AS documentos ON clientes.documcliente = documentos.coddocumento
	LEFT JOIN mediospagos ON ventas.formapago = mediospagos.codmediopago 
	LEFT JOIN usuarios ON ventas.codigo = usuarios.codigo WHERE ventas.codigo = '".limpiar($_SESSION["codigo"])."' AND DATE_FORMAT(ventas.fechaventa,'%d-%m-%Y') = '".date("d-m-Y")."' GROUP BY detalleventas.codventa";
	foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
...
###################### FUNCION BUSQUEDA VENTAS POR CAJAS ###########################
	public function BuscarVentasxCajas() 
	{
		...
		LEFT JOIN clientes ON ventas.codcliente = clientes.codcliente
		LEFT JOIN documentos AS documentos ON clientes.documcliente = documentos.coddocumento
		 WHERE ventas.codcaja = ? AND DATE_FORMAT(ventas.fechaventa,'%Y-%m-%d') >= ? AND DATE_FORMAT(ventas.fechaventa,'%Y-%m-%d') <= ? GROUP BY detalleventas.codventa";
		$stmt = $this->dbh->prepare($sql);
		$stmt->bindValue(1, trim(decrypt($_GET['codcaja'])));
		$stmt->bindValue(2, trim(date("Y-m-d",strtotime($_GET['desde']))));
		$stmt->bindValue(3, trim(date("Y-m-d",strtotime($_GET['hasta']))));
...
###################### FUNCION BUSQUEDA VENTAS POR FECHAS ###########################
	public function BuscarVentasxFechas() 
	{
		...
		LEFT JOIN clientes ON ventas.codcliente = clientes.codcliente
		LEFT JOIN documentos AS documentos ON clientes.documcliente = documentos.coddocumento
		 WHERE DATE_FORMAT(ventas.fechaventa,'%Y-%m-%d') >= ? AND DATE_FORMAT(ventas.fechaventa,'%Y-%m-%d') <= ? GROUP BY detalleventas.codventa";
		$stmt = $this->dbh->prepare($sql);
		$stmt->bindValue(1, trim(date("Y-m-d",strtotime($_GET['desde']))));
		$stmt->bindValue(2, trim(date("Y-m-d",strtotime($_GET['hasta']))));
		$stmt->execute();
...
########################## FUNCION REGISTRAR PAGOS A CREDITOS EN RESERVACIONES ########################
	public function RegistrarPagoReservaciones()
	{
	    ...
	    else if(empty($_POST["codcliente"]) or empty($_POST["codreservacion"]) or empty($_POST["montoabono"]))
		{
			echo "2";
			exit;
		} 
...
public function VerDetallesAbonos()
{
	...
	$sql = "SELECT * FROM abonoscreditosreservaciones INNER JOIN reservaciones ON abonoscreditosreservaciones.codreservacion = reservaciones.codreservacion LEFT JOIN cajas ON abonoscreditosreservaciones.codcaja = cajas.codcaja WHERE abonoscreditosreservaciones.codreservacion = ?";	
	$stmt = $this->dbh->prepare($sql);
	...
	else
	{
		...
		$sql = "SELECT * FROM abonoscreditosventas INNER JOIN ventas ON abonoscreditosventas.codventa = ventas.codventa LEFT JOIN cajas ON abonoscreditosventas.codcaja = cajas.codcaja WHERE abonoscreditosventas.codventa = ?";	
	$stmt = $this->dbh->prepare($sql);
	$stmt->bindValue(1, trim(decrypt($_GET["codproceso"])));
	$stmt->execute();
	$num = $stmt->rowCount();
...
###################### FUNCION BUSQUEDA CREDITOS POR CLIENTES ###########################
	...
	LEFT JOIN abonoscreditosreservaciones ON reservaciones.codreservacion = abonoscreditosreservaciones.codreservacion
	LEFT JOIN documentos ON clientes.documcliente = documentos.coddocumento
		WHERE reservaciones.codcliente = ? AND reservaciones.tipopago ='CREDITO' GROUP BY reservaciones.codreservacion";
		$stmt = $this->dbh->prepare($sql);
		$stmt->bindValue(1, trim($_GET['codcliente']));
		$stmt->execute();
		$num = $stmt->rowCount();
...
###################### FUNCION BUSQUEDA CREDITOS POR FECHAS ###########################
	...
	LEFT JOIN abonoscreditosreservaciones ON reservaciones.codreservacion = abonoscreditosreservaciones.codreservacion
	LEFT JOIN documentos ON clientes.documcliente = documentos.coddocumento
	WHERE DATE_FORMAT(reservaciones.fecharegistro,'%Y-%m-%d') >= ? AND DATE_FORMAT(reservaciones.fecharegistro,'%Y-%m-%d') <= ?
	 AND reservaciones.tipopago ='CREDITO' GROUP BY reservaciones.codreservacion";
		$stmt = $this->dbh->prepare($sql);
		$stmt->bindValue(1, trim(date("Y-m-d",strtotime($_GET['desde']))));
		$stmt->bindValue(2, trim(date("Y-m-d",strtotime($_GET['hasta']))));
		$stmt->execute();
...
public function ListarMensajes()
{
	...
	if($_SESSION['acceso'] == "cliente") {
		...
		$sql = "SELECT * FROM mensajes LEFT JOIN clientes ON mensajes.codigo = clientes.codcliente LEFT JOIN usuarios ON mensajes.codigo = usuarios.codigo WHERE mensajes.respuesta = '".limpiar($_SESSION["codcliente"])."' ORDER BY mensajes.codmensaje ASC";
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
	...
	$sql = "SELECT * FROM mensajes LEFT JOIN clientes ON mensajes.codigo = clientes.codcliente LEFT JOIN usuarios ON mensajes.codigo = usuarios.codigo ORDER BY mensajes.codmensaje ASC";
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
...
public function ListarClientesActivos()
	{
		self::SetNames();
		$sql = "SELECT * FROM clientes INNER JOIN reservaciones ON clientes.codcliente = reservaciones.codcliente WHERE reservaciones.reservacion = 'ACTIVA' GROUP BY reservaciones.codcliente";
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
 ...
 public function SumaReservaciones()
{
	...
	SUM(totalpago) totalmes
	FROM reservaciones 
	WHERE YEAR(fecharegistro) = '".date('Y')."' AND MONTH(fecharegistro) GROUP BY MONTH(fecharegistro) ORDER BY 1";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
public function HabitacionesMasReservadas()
	...
	self::SetNames();
     ...
     $sql = "SELECT habitaciones.codhabitacion, habitaciones.numhabitacion, COUNT(detallereservaciones.codhabitacion) as cantidad FROM (reservaciones LEFT JOIN detallereservaciones ON reservaciones.codreservacion=detallereservaciones.codreservacion) LEFT JOIN habitaciones ON detallereservaciones.codhabitacion=habitaciones.codhabitacion GROUP BY detallereservaciones.codhabitacion ORDER BY habitaciones.codhabitacion ASC LIMIT 8";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
(SELECT COUNT(codproducto) FROM productos WHERE existencia <= stockminimo AND codsucursal = '".$_SESSION["codsucursal"]."') AS minimo,
(SELECT COUNT(codproducto) FROM productos WHERE fechaexpiracion != '0000-00-00' AND fechaexpiracion <= '".date("Y-m-d")."' AND codsucursal = '".$_SESSION["codsucursal"]."') AS vencidos,

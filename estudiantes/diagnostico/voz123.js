window.addEventListener('load', function() {
	...
	navigator.mediaDevices.getUserMedia({audio: true}).then (function (stream) {
		...
		recognition.onresult = function (event) {
			...
			if (answer == secuencia1234[group][cardNo].word || secuencia1234[group][cardNo].alt &&secuencia1234[group][cardNo].alt.indexOf(answer) != -1) {
				if (cardNo < secuencia1234[group].length - 1) {
					$success.play();
					showCard(cardNo + 1);
				} else {
					$tada.play();
	...
	function showCard(no) {
		cardNo = no;
		var curr = secuencia1234[group][cardNo];

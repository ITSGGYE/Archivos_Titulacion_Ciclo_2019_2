...
switch ($_GET["accion"])
{
	...
	case "generar_calendario":
	{
		...
		$eventos=array();
		//$query=$db->query("select * from ".$tabla." where month(desde)='".$fecha_calendario[1]."' and year(desde)='".$fecha_calendario[0]."' and codhabitacion = '".$_GET["codigo"]."'");
		
		$query=$db->query("select desde, hasta from ".$tabla." where codhabitacion = '".decrypt($_GET["codigo"])."'");
		...
		//echo "<h5 class='portlet-title text-dark text-uppercase'><div align='left'><a href='#' rel='$mesanterior' class='anterior'><div class='btn-group m-b-10'><button type='button' class='btn btn-default waves-effect'><i class='fa fa-chevron-left'></i><span></button></a><a href='#' class='siguiente' rel='$messiguiente'><button type='button' class='btn btn-default waves-effect'><i class='fa fa-chevron-right'></i></button></a></div>"."&nbsp;&nbsp;<strong>".$meses[intval($fecha_calendario[1])]." ".$fecha_calendario[0]." </strong></h5> ";

                       ...
                       $selePac = $bd->prepare("SELECT id_paciente, nombre_apellido FROM paciente WHERE id_paciente = '".$nombre."'"); 
                        ...
                        {   
                         ...
                         if($nombre === $idPaciente){ 
                                  ...
                                  $citCanc = $bd->prepare("SELECT * FROM cita  WHERE  estado = 'cancelado'  AND id_especialidad =  '".$nombrecapa."' AND id_especialista = '".$curso."' AND fecha = '".$fecha."' AND hora = '".$dates."'"); 
                                $citCanc->execute(); 
                                $users = $citCanc->fetchAll(); 
                                foreach($users as $user)  
                                {   
                                  $idCita = $user['id_cita'];

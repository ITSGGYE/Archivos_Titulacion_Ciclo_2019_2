        ...
        swal({
          ...
          if(data==1){
            ...
            $("#savetipos")[0].reset();
             ...
             swal("Oops", "Este Tipo de Habitación no puede ser Eliminada, tiene registros relacionados!", "error"); 
  ...
  direccliente,telefcliente,correocliente,limitecredito,proceso) 
{
  ...
  $("#saveclientes #paiscliente").val(paiscliente);
  $("#saveclientes #ciudadcliente").val(ciudadcliente);
  $("#saveclientes #direccliente").val(direccliente);
  $("#saveclientes #telefcliente").val(telefcliente);
  $("#saveclientes #correocliente").val(correocliente);
...
{
    // aqui asigno cada valor a los campos correspondientes
  $("#saveinsumos #codinsumo").val(codinsumo);
  $("#saveinsumos #insumo").val(insumo);
  $("#saveinsumos #codcategoria").val(codcategoria);
  $("#saveinsumos #preciocompra").val(preciocompra);
  $("#saveinsumos #precioventa").val(precioventa);
  $("#saveinsumos #existencia").val(existencia);
  $("#saveinsumos #stockminimo").val(minimo);
  $("#saveinsumos #ivainsumo").val(ivainsumo);
  $("#saveinsumos #descinsumo").val(descinsumo);
  $("#saveinsumos #fechaexpiracion").val(fechaexpiracion);
  $("#saveinsumos #codproveedor").val(codproveedor);
  $("#saveinsumos #lote").val(lote);
  $("#saveinsumos #proceso").val(proceso);
}
...
function UpdateSalidaInsumo(codsalida,codinsumo,busquedainsumo,responsable,existencia,cantsalida,proceso) 
{
    // aqui asigno cada valor a los campos correspondientes
...
var dataString = "Productos_Categorias=si";
...
function CondicionesPagosWeb(){

var tipopago = $('select#tipopago').val(); 

var dataString = 'BuscaCondicionPago=si&tipopago='+tipopago;
...
$.ajax({
            ...
            data: dataString,
            success: function(response) {            
                $('#muestratipopago').empty();
                $('#muestratipopago').append(''+response+'').fadeIn("slow");
            }
      });
}
...
var url = 'funciones.php?BuscarHabitaciones=si';
...
function CargaTab(tipopago){

$('#muestratabs').html('<center><i class="fa fa-spin fa-spinner"></i> Cargando información, por favor espere....</center>');

var dataString = 'BuscaTabsReservacion=si&tipopago='+tipopago;
...
//FUNCION PARA CALCULADR MONTO DEVOLUCION EN VENTA
$(document).ready(function (){
...
function AbonoCreditoReservacion(codcliente,codreservacion,totaldebe,dnicliente,nomcliente,nroreservacion,totalfactura,fechaventa,totalabono,debe) 
{
    // aqui asigno cada valor a los campos correspondientes
  ...
  $("#savepagoreservaciones #debe").val(debe);
}
...
function AbonoCreditoVenta(codcliente,codventa,totaldebe,dnicliente,nomcliente,nroventa,totalfactura,fechaventa,totalabono,debe) 
{
    // aqui asigno cada valor a los campos correspondientes
